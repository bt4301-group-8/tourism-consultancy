    ...
    try:
        ...
        if text and title:
            review_data_dict = {
                ...
                "rating": rating,
                # format trip date from e.g. "Apr 2024" to "Apr-24"
                "trip_date": trip_date.split()[0] + "-" + trip_date.split()[1][-2:] if trip_date else None,
...
# dag definition
@dag(
    dag_id=f"tripadvisor_{COUNTRY_NAME}_scraper_demo",
    schedule_interval=None, # none for manual runs
    start_date=pendulum.datetime(2025, 4, 1),
    catchup=False,
    tags=["tripadvisor", "scraping", "singapore", "demo"],
    default_args={
        "retries": 1,
    ...
    - Scrapes reviews for using curl_cffi
...
def tripadvisor_singapore_scraper():
    ...
    def load_reviews_to_mongodb(reviews, target_period):
        ...
        try:
            ...
            db = client.demo
            review_collection = db.posts.tripadvisor
